...
export default function CalcoloUltimoMiglioPensione() {
  ...
  const vociArricchite = useMemo(() => {
    return voci.map(v => {
      ...
      const importoMensile = round2(v.importoMensile);
      return {
        ...v,
        importoMensile,
        catalogo,
        importoAnnuo: catalogo ? round2(importoMensile * catalogo.moltiplicatore) : 0
      };
    });
  }, [voci]);

  const totaleVociFisseAnnuo = useMemo(() => {
    return round2(vociArricchite.reduce((sum, v) => sum + v.importoAnnuo, 0));
  }, [vociArricchite]);

  const totaleTredicesimaMensilita = useMemo(() => {
    return round2(
      vociArricchite.reduce((sum, v) => {
                      ...
                      <tr key={v.idRiga} className={i % 2 === 0 ? styles.rowEven : ''}>
                        ...
                        <td>
                          <input
                            ...
                            placeholder="0,00"
                            onChange={e => updateRow(v.idRiga, 'importoMensile', round2(parseFloat(e.target.value) || 0))}
                          />
                        </td>
