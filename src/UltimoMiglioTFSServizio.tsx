...
interface Anagrafica {
  ...
  dataInizio: string;
  dataFine: string;
  motivoCessazione: string; // FIX: aggiunto campo richiesto dalle funzioni di export
}
...
const VOCI_RETRIBUTIVE = [
  ...
  { id: 'v8', label: 'Indennita specifica 5,38', valido13: false },
  { id: 'v9', label: 'Indennita Vacanza Contrattuale', valido13: true },
  { id: 'v10', label: 'Indennita di vigilanza', valido13: false },
  { id: 'v11', label: 'Indennita Professionale asili nido', valido13: false },
  { id: 'v12', label: 'Indennita Aggiuntiva asili nido', valido13: false },
];

interface VoceInput { id: string; importoMensile: number; }
...
export default function UltimoMiglioTFSServizio() {
  ...
  const mesiCalcolati = useMemo(() => {
    return stipendiMensili.map((importo, i) => {
      ...
      return { meseId: i + 1, importoMensileEffettivo: effettivo, quota13: round2(effettivo / 12) };
    });
  }, [importoBaseMensileGlobale, stipendiMensili, eccezioni]);

  const totale13Step2 = useMemo(() =>
    round2(mesiCalcolati.reduce((sum, m) => sum + m.quota13, 0)),
    [mesiCalcolati]
  );
  ...
  const vociCalcolate = useMemo(() => {
    return VOCI_RETRIBUTIVE.map(v => {
      ...
      let importoAnnuo = 0;
      if (v.id === 'v1') { importoMensile = round2(importoBaseMensileGlobale); importoAnnuo = round2(importoMensile * 12); }
      else if (v.id === 'v2') { importoMensile = round2(totale13Step2); importoAnnuo = importoMensile; }
      else { importoAnnuo = round2(importoMensile * 12); }
  ...
  return (
    <div className={styles.root}>
      <aside className={styles.sidebar}>
        <div className={styles.logo}><span className={styles.logoText}>Immedia</span><span className={styles.logoSub}>S.p.A.</span></div>
        <div className={styles.appTitle}><span>Calcolo</span><span className={styles.highlight}>Ultimo Miglio</span><span>TFS Servizio</span></div>
        <nav className={styles.nav}>
          {STEPS.map((s, i) => (
            <button key={s.id} className={`${styles.navItem} ${step === s.id ? styles.navActive : ''} ${i < currentStepIdx ? styles.navDone : ''}`}
              onClick={() => i <= currentStepIdx ? go(s.id) : undefined} disabled={i > currentStepIdx}>
              <span className={styles.navNum}>{i < currentStepIdx ? '✓' : s.num}</span>
              <span>{s.label}</span>
            </button>
          ))}
        </nav>
        <div className={styles.sidebarFooter}><span>Enti Locali - INPS</span><span>Personale in Servizio</span></div>
      </aside>
        ...
        <div className={styles.content}>
            ...
            <div className={styles.card}>
              <div className={styles.grid2}>
                <div className={styles.field}>
                  ...
                  <input className={styles.input} value={anagrafica.cognomeNome} onChange={e => setAnagrafica({ ...anagrafica, cognomeNome: e.target.value })} placeholder="Es. Mario Rossi" />
                </div>
                <div className={styles.field}>
                  <label className={styles.label}>Codice Fiscale <span className={styles.required}>*</span></label>
                  <input className={styles.input} value={anagrafica.codiceFiscale} onChange={e => setAnagrafica({ ...anagrafica, codiceFiscale: e.target.value.toUpperCase().slice(0, 16) })} placeholder="16 caratteri" maxLength={16} />
                </div>
                <div className={styles.field}>
                  <label className={styles.label}>Data Inizio Periodo</label>
                  <input className={styles.input} type="date" value={anagrafica.dataInizio} onChange={e => setAnagrafica({ ...anagrafica, dataInizio: e.target.value })} />
                </div>
                <div className={styles.field}>
                  <label className={styles.label}>Data Fine Periodo</label>
                  <input className={styles.input} type="date" value={anagrafica.dataFine} onChange={e => setAnagrafica({ ...anagrafica, dataFine: e.target.value })} />
                </div>
                <div className={styles.field}>
                  <label className={styles.label}>Motivo Cessazione</label>
                  <select className={styles.input} value={anagrafica.motivoCessazione} onChange={e => setAnagrafica({ ...anagrafica, motivoCessazione: e.target.value })}>
              ...
              <div className={styles.actions}>
                <button className={styles.btnPrimary} onClick={() => go('stipendi')} disabled={!anagrafica.cognomeNome || !anagrafica.codiceFiscale}>Continua</button>
              </div>
            </div>
          )}
          ...
          {step === 'stipendi' && (
            <div className={styles.card}>
              <div className={styles.baseInputGroup}>
                <div className={styles.field}>
                  <label className={styles.label}>Importo Base Mensile Globale (EUR)</label>
                  <div style={{ display: 'flex', gap: '1rem' }}>
                    <input className={styles.inputNum} type="number" min={0} step="0.01" value={importoBaseMensileGlobale || ''} onChange={e => setImportoBaseMensileGlobale(round2(parseFloat(e.target.value) || 0))} placeholder="0,00" />
                    <button className={styles.btnSecondary} onClick={handleApplicaBase}>Applica a tutti i mesi</button>
                  </div>
                </div>
              </div>
              <div className={styles.tableWrap}>
                <table className={styles.table}>
                  <thead><tr><th>Mese</th><th>Eccezione</th><th>Importo Mensile Effettivo</th><th>Quota 13^</th></tr></thead>
                  <tbody>
                    {mesiCalcolati.map((m, i) => (
                      <tr key={i} className={i % 2 === 0 ? styles.rowEven : ''}>
                        <td className={styles.tdLabel}>Mese {m.meseId}</td>
                        <td className={styles.tdCenter}>
                          <input type="checkbox" checked={eccezioni[i]} onChange={e => {
                            const newEcc = [...eccezioni]; newEcc[i] = e.target.checked; setEccezioni(newEcc);
                            if (!e.target.checked) { const newStip = [...stipendiMensili]; newStip[i] = round2(importoBaseMensileGlobale); setStipendiMensili(newStip); }
                          }} />
                        </td>
                        <td>
                          <input className={styles.inputNum} type="number" min={0} step="0.01"
                            value={eccezioni[i] ? (stipendiMensili[i] || '') : importoBaseMensileGlobale} disabled={!eccezioni[i]}
                            onChange={e => { const val = round2(parseFloat(e.target.value) || 0); const newStip = [...stipendiMensili]; newStip[i] = val; setStipendiMensili(newStip); }} />
                        </td>
          ...
          {step === 'voci' && (
            <div className={styles.card}>
              <div className={styles.tableWrap}>
                <table className={styles.table}>
                  <thead><tr><th>Voce Retributiva</th><th style={{textAlign:'center'}}>Valido 13^</th><th style={{textAlign:'right'}}>Importo Mensile</th><th style={{textAlign:'right'}}>Importo Annuo</th></tr></thead>
                  <tbody>
                    {vociCalcolate.map((v, i) => {
                      ...
                      return (
                        <tr key={v.id} className={i % 2 === 0 ? styles.rowEven : ''}>
                          ...
                          <td className={styles.tdCenter}>{v.valido13 ? 'SI' : '-'}</td>
                          <td className={styles.tdRight}>
                            {isReadOnly ? <div className="text-slate-500 font-medium">{fmtEuro(v.importoMensile)}</div>
                              : <input className={styles.inputNum} type="number" min={0} step="0.01" value={voci.find(x => x.id === v.id)?.importoMensile || ''} placeholder="0,00"
                                  onChange={e => { const val = round2(parseFloat(e.target.value) || 0); setVoci(prev => prev.map(x => x.id === v.id ? { ...x, importoMensile: val } : x)); }} />}
                          </td>
          ...
          {step === 'risultato' && (
            <div>
              ...
              <div className={styles.card}>
                ...
                <table className={styles.table}>
                  <thead><tr><th>Componente</th><th style={{ textAlign: 'right' }}>Importo Annuo</th></tr></thead>
                  <tbody>
                    {[
                      { label: 'Stipendio tabellare nuove Aree Tab G', val: risultato.r1 },
                      { label: 'Retribuzione Ind. di Anzianita RIA', val: risultato.r2 },
                      { label: 'Tredicesima mensilita', val: risultato.r3 },
                      { label: 'Assegno ad personam assorbibile progressione verticale', val: risultato.r4 },
                      { label: 'Indennita Aggiuntive personale asili nido', val: risultato.r5 },
                      { label: 'Assegno personale non riassorbibile art 29', val: risultato.r6 },
                      { label: 'Indennita 64,56 annue lorde ex 3^ 4^ qualifica', val: risultato.r7 },
                      { label: 'Indennita di vigilanza per 12 mensilita', val: risultato.r8 },
              ...
              <div className={styles.actions} style={{ marginTop: 24 }}>
                ...
                <button className={styles.btnExport} onClick={() => exportTFSServizioToExcel(anagrafica, vociCalcolate, risultato, mesiCalcolati, totale13Step2)}>Esporta Excel</button>
                <button className={styles.btnPdf} onClick={() => exportTFSServizioToPDF(anagrafica, vociCalcolate, risultato, mesiCalcolati, totale13Step2)}>Esporta PDF</button>
              </div>
            </div>
          )}
        </div>
