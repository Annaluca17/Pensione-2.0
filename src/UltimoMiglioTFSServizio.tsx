...
export default function UltimoMiglioTFSServizio() {
  ...
  const mesiCalcolati = useMemo(() => {
    return stipendiMensili.map((importo, i) => {
      ...
      return {
        ...
        quota13: round2(effettivo / 12)
      };
    });
  }, [importoBaseMensileGlobale, stipendiMensili, eccezioni]);

  const totale13Step2 = useMemo(() => {
    return round2(mesiCalcolati.reduce((sum, m) => sum + m.quota13, 0));
  }, [mesiCalcolati]);
  ...
  const risultato = useMemo(() => {
    ...
    const r3 = round2(
      getAnnuo('v2') + getMensile('v3') + getMensile('v4') + getMensile('v5') +
      getMensile('v6') + getMensile('v7') + getMensile('v9')
    );
