...
export default function UltimoMiglioTFSPensione() {
  ...
  const applicaBase = useCallback(() => {
    setStipendiMensili(prev =>
      prev.map((v, i) => eccezioni[i] ? v : { stipendio: round2(importoBaseStipendio), tredicesima: round2(importoBaseTredicesima) })
    );
          ...
          {step === 'anagrafica' && (
            <div className={styles.card}>
              <div className={styles.grid2}>
                ...
                <FormField label="Motivo Cessazione">
                  <select
                    className={styles.input}
                    value={anagrafica.motivoCessazione}
                    onChange={e => setAnagrafica(a => ({ ...a, motivoCessazione: e.target.value }))}
                  >
          ...
          {step === 'stipendi' && (
            <div className={styles.card}>
              ...
              <div className={styles.totaleMesiRow}>
                ...
                <span style={{ marginLeft: 24 }}>Totale 13^ mensilità:</span>
                    ...
                    <tbody>
                      {risultato.vociAnnualizzate.map((v, i) => (
                        <tr key={v.id} className={i % 2 === 0 ? styles.rowEven : ''}>
                          ...
                          <td className={styles.tdRight}>{fmtEuro(v.importoAnnuo ?? 0)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
